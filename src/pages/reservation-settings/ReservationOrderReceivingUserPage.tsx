...
const ReservationOrderReceivingUserPage: React.FC = () => {
  const { employees, reservationOrderReceivingUserIds, setReservationOrderReceivingUserIds } = useRestaurantData();
  const [selectedIds, setSelectedIds] = useState<string[]>(reservationOrderReceivingUserIds);
  const [showSavedMessage, setShowSavedMessage] = useState(false);

  useEffect(() => {
    setSelectedIds(reservationOrderReceivingUserIds);
  ...
  const handleSelectAll = () => {
    setSelectedIds(activeEmployees.map(emp => emp.id));
  };
  ...
  const handleSave = () => {
    setReservationOrderReceivingUserIds(selectedIds);
    setShowSavedMessage(true);
    setTimeout(() => setShowSavedMessage(false), 2500);
  };
  
  const isDirty = JSON.stringify([...selectedIds].sort()) !== JSON.stringify([...reservationOrderReceivingUserIds].sort());

  return (
    <div className="p-4 sm:p-6 space-y-6">
      <Card title="Reservation Order Receiving Users" icon={<FiUsers />}>
        <div className="p-4 border-t border-gray-200">
          <p className="text-sm text-gray-500 mb-4">
            Select which employees should be notified or assigned when a new pre-order is placed with a reservation.
          </p>
          ...
          <div className="space-y-2 border rounded-lg p-2 max-h-[50vh] overflow-y-auto custom-scrollbar">
            {activeEmployees.length > 0 ? (
              activeEmployees.map(emp => (
                <label key={emp.id} className="flex items-center p-3 hover:bg-gray-100 rounded-md cursor-pointer transition-colors">
                  <input
                    type="checkbox"
...
export default ReservationOrderReceivingUserPage;