...
import { FiSearch, FiCalendar, FiXCircle, FiEye, FiShoppingCart, FiDollarSign, FiArrowLeft } from 'react-icons/fi';
import Money from '@/components/common/Money';

const ORDER_TYPES = ["All", "Dine In", "Delivery", "Pickup", "WhatsApp"];

const DetailedSaleReportPage: React.FC = () => {
  ...
  const handleDownload = (format: 'PDF' | 'Excel' | 'CSV') => {
    alert(`Downloading Detailed Sale Report as ${format}... (This is a simulation)`);
  };
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <h1 className="text-2xl sm:text-3xl font-semibold text-gray-800 flex items-center">
            <FiShoppingCart className="mr-3 text-sky-600"/> Detailed Sale Report
        </h1>
            ...
            <table className="w-full min-w-max">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredSales.map(sale => (
                  <tr key={sale.id} className="hover:bg-sky-50 transition-colors">
...
export default DetailedSaleReportPage;
